...
public class SwingState : StateBase
{
    ...
    public override void ExitState()
    {
        ...
        _fsm.transform.Rotate(Quaternion.LookRotation(_fsm.transform.forward.projectedOnPlane(Vector3.up), Vector3.up).eulerAngles, 0.2f);
    ...
    {
        if(Physics.Raycast(_fsm.transform.position, Vector3.down, 0.3f, _blackBoard.groundLayer))
        {
            return true;
        }
        else
